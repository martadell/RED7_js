class Palette {
  ...
  sortPalette() {
    //ordena la palette del 7 a l'1 (i de vermell a lila)
  ...
  calcHighest() {
    //red rule: return la carta més alta
    return this._cards[0]; //com està ordenat per valors treu la primera
  }

  calcSameNum() {
    //orange rule: return les maximes cartes repetides (mateix numero)
    let prevCard = this._cards[0];
    let numbers = [[]];

    this._cards.forEach((card) => {
      //1. fem un array d'arrays amb cartes del mateix numero
  ...
  calcSameColor() {
    //yellow rule: return maxim cartes amb mateix color (igual que l'anterior pero amb color)
    this.sortByColor(); //ordenem per colors
    let colors = [[]];
    let prevCard = this._cards[0];

    this._cards.forEach((card) => {
      //1. fem un array d'arrays amb cartes del mateix color
  ...
  calcEvens() {
    //green rule: return les cartes parells
    let evenCards = [];
  ...
  calcDifColor() {
    //blue rule: return cartes amb diferents colors
    this.sortByColor(); //ordenem per colors
    ...
    this._cards.forEach((card) => {
      ...
      if (card.color != prevCard.color) {
        //si al nostre array no hi ha carta de color se li fica
  ...
  calcRun() {
    //indigo rule: return cartes de l'escala més llarga
    let runs = [];
    let prevCard = this._cards[0];

    this._cards.forEach((card) => {
      //1. fem un array amb les diferents escales
  ...
  calcBellowFour() {
    //purple rule: return cartes que tenen com a valor <4
    let bellowFourCards = [];
